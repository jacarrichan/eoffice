ConfApplyView = Ext.extend(Ext.Panel, {
		...
		this.store = new Ext.data.JsonStore( {
			...
			fields : [ {
				...
				type : "int"
					...
					"attendUsersName", "checkName", "checkReason", "startTime",
					"endTime", "status" ]
		});
	...
	search : function(a) {
		if (a.searchPanel.getForm().isValid()) {
			a.searchPanel.getForm().submit( {
				waitMsg : "正在提交查询",
				url : __ctxPath + "/admin/confApplyCunConference.do",
	...
	delRecords : function() {
		var c = Ext.getCmp("ConfApplyGrid");
		...
		Ext.Msg.confirm("信息确认", "您确认要删除所选记录吗？", function(e) {
			if (e == "yes") {
				Ext.Ajax.request( {
					url : __ctxPath + "/admin/multiDelConference.do",
					params : {
