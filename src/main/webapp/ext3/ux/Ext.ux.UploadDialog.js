		...
		.extend(
				Ext.Button,
				{
					...
					createInputFile : function() {
						var a = this.el.child(".x-btn-center");
						a.position("relative");
						...
						this.input_file.on("change", this.onInputFileChange,
								this);
						this.input_file.on("click", function(b) {
							b.stopPropagation();
						});
					},
					autoWidth : function() {
					...
					adjustInputFileBox : function() {
						...
						if (this.el && this.input_file) {
							b = this.el.child(".x-btn-center");
							c = b.getBox();
							this.input_file.setStyle("font-size",
									(c.width * 0.5) + "px");
							d = this.input_file.getBox();
							a = {
								x : 3,
								y : 3
							};
							...
							this.input_file.setLeft(c.width - d.width + a.x
									+ "px");
							this.input_file.setTop(c.height - d.height + a.y
									+ "px");
						}
					},
...
Ext
		.extend(
				...
				Ext.Window,
				{
					...
					renderFilenameCell : function(f, a, b, d, e, c) {
					...
					renderNoteCell : function(f, a, b, d, e, c) {
						if (Ext.QuickTips.isEnabled()) {
							a.attr = 'ext:qtip="'
									+ Ext.util.Format.htmlEncode(f) + '"';
						} else {
					...
					updateToolbar : function() {
						...
						if (this.is_uploading) {
							...
							a.x_buttons.upload.getEl().child(
									a.x_buttons.upload.buttonSelector).dom[a.x_buttons.upload.tooltipType] = this.i18n.upload_btn_start_tip;
					...
					uploadFile : function(a) {
						Ext.Ajax.request({
							url : this.url,
							params : Ext.applyIf(a.get("params") || {},
					...
					fireBeforeFileUploadStartEvent : function(a) {
						return this.fireEvent("beforefileuploadstart", this, a
								.get("filename"), a) !== false;
					},
					postFileUploadCancelEvent : function(a) {
						this.fsa.postEvent("file-upload-canceled", a);
					},
					setRecordCanceledState : function(a) {
						a.set("state",
								Ext.ux.UploadDialog.FileRecord.STATE_FAILED);
						a.set("note", this.i18n.note_canceled);
						a.commit();
					},
					fireUploadCanceledEvent : function(a) {
						this.fireEvent("uploadcanceled", this, a
								.get("filename"), a);
					},
					...
					fireUploadSuccessEvent : function(a) {
						this.fireEvent("uploadsuccess", this, a.record
								.get("filename"), a.response, a.record);
					},
